...
const main = async () => {
  ...
  const loanPoolFactory = await hre.ethers.getContractFactory("LoanPool");
  ...
  const amount = "155";
  const txn = await loanPool.deposit({ value: ethers.utils.parseEther(amount) });
  await txn.wait();
  console.log(`deposited ${amount} FIL`);

  const amount2 = "178";
  const txn2 = await loanPool.connect(wallet2).deposit({ value: ethers.utils.parseEther(amount2) });
  await txn2.wait();
  console.log(`deposited ${amount2} FIL by other wallet`);

  const amount4 = ethers.utils.formatEther(await loanPool.totalDeposit());
